...
export default function UrlEditor({
  ...
  return (
    <>
      <form className='flex'>
        <select
          className='px-4 py-2 border text-sm font-bold rounded-md border-gray-300 hover:border-orange-500 focus:outline-none bg-white'
          value={reqMethod}
          onChange={(e) => setReqMethod(e.target.value)}
        >
          {requestMethods.map((option) => (
            <option className='font-bold' key={option.slug} value={option.method}>
              {option.method}
            </option>
          ))}
        </select>
